...
// A silent socket must never be reported as the machine having timed out —
// only as lost contact, distinctly.
...
// A sleeping DE1 drops the cleaning/descaling request, so the page wakes it
// first. Only a confirmed sleep earns a wake: guessing on an unknown state
// would just delay the real request.
...
// carries, so the two cycles read the same way in every language. Only the two
// failure sentences are new; every label and every other caption reuses a row
