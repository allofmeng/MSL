// Per-profile tile edits (dose / yield / grind) live in Decaid's KV store, not
// on the profile record's metadata: Decaid replaces that map when it re-seeds a
// bundled profile, which silently wiped the user's numbers.
...
test('the KV namespace keeps its persisted streamline* spelling', () => {
    // Renaming it orphans every user's saved dose/yield/grind — see CLAUDE.md.
    assert.equal(OVERRIDES_NAMESPACE, 'streamlineProfileOverrides');
});
...
test("only the three tile values are stored — never Decaid's own metadata", async () => {
    // MSL has no brew-temperature tile: the keys are exactly the three fields
    // saveContextToActiveProfile writes.
    fakeKv();
    await loadProfileOverrides();
    ...
    assert.deepEqual(saved, { targetDoseWeight: 18 });
});
