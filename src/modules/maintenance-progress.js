...
// firmware; the skin only watches it happen. The snapshot socket reports the
...
//    "not running" flashes a false "finished" the moment Start is pressed.
...
//
// Substate spellings come from Decaid's MachineSubstate enum, not from
// rest_v1.yml — the spec's `cleaingGroup` is a typo that never appears on the
// wire.
//
// DOM-free on purpose (see CLAUDE.md: pure modules stay unit-testable);
// settings.js owns the DOM rendering.
...
/** How long to give a sleeping machine to wake before sending the start request anyway. */
 ...
 * `cleaningGroup`, so the wire cannot tell fill from flush on its own. Order
 * does: a `cleaningGroup` before the soak is the fill, after it is the flush.
 * Joining a cycle already in progress on a `cleaningGroup` frame is genuinely
 * ambiguous and is shown as the fill.
 */
 ...
 * descaleGroup all onto `cleaningGroup`, and unlike the cleaning cycle nothing
 * else arrives in between to separate them by order. The middle of a descale
 * is genuinely one indistinguishable phase over this API.
 */
 ...
 * why nothing separate is needed to adopt a run already in progress.
 *
...
/**
 * Whether to send a wake before the start request. Only a confirmed `sleeping`
 * earns one — an unknown state (no snapshot yet) does not, since a wake there
 * would be guesswork that delays the actual request, and the cycle's own entry
 * timeout already reports a machine that never started.
 */
