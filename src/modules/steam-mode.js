...
// The Decaid name a Bengle's onboard milk probe registers under once attached
// (BengleMilkProbe.info.name, auto-registered by BengleProbeBridge — GET
// /api/v1/sensors is empty until then). Matched on name rather than vendor
// because the sensor list's `id` is an opaque, undocumented Decaid
// implementation detail (`${machineDeviceId}-milkprobe`) that must not be
// relied on as a stable contract.
