...
//
// A ShotSnapshot's `machine` and `scale` are both optional, so a snapshot may
// carry a scale reading and no machine frame. Such a frame must INHERIT the
// phase of the last machine frame seen, not be dropped for having none --
// dropping them would thin or empty the GFlow trace on any record whose scale
// and machine frames are not paired one-to-one.
//
