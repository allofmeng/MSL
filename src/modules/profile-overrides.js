// Per-profile tile edits (dose / yield / grind), kept in Decaid's KV store and
// keyed by profile id.
//
...
//
// The namespace name is a persisted identifier and stays spelled `streamline*`
// for the same reason every other one does — see CLAUDE.md.
//
...
// The tile values a user can override. These are exactly the three fields MSL
// writes through saveContextToActiveProfile (dose, yield, grind) — it has no
// brew-temperature tile. Anything else in a record's metadata (source,
// filename, ...) is Decaid's and is never written here.
export const OVERRIDE_KEYS = ['targetDoseWeight', 'targetYield', 'grinderSetting'];
