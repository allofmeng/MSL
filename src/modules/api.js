...
export function connectWebSocket(onData, onReconnect) {
    ...
    reconnectingWebSocket.onmessage = (event) => {
        try {
            ...
            // Suppressed = a foreground op (firmware flash) is why the machine
            // slept. Skip the dim; the 'restore' side still runs.
...
// single JSON array per key under this namespace. MSL is a read-only
...
// actually resolves the in-flight uploadFirmware promise.
