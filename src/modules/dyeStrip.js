// dyeStrip.js — Phase B of the DYE2 → MSL dashboard bridge.
//
// MSL's header carries a vertical P / F / R toggle (top-left):
//   P = Profile favourites  (the existing profile <nav>, untouched — DEFAULT)
//   F = DYE2 auto-favourites (bean + recipe snapshots)
//   R = DYE2 recipes
// F and R are driven by data DYE2 writes to the generic KV store; MSL is a
...
// MSL (profile favourites only, no toggle / DYE button / strip).
...
function wireOnce() {
    ...
    // DYE2 made while MSL was idle appear (KV_CONTRACT "Freshness").
    const repoll = () => {
