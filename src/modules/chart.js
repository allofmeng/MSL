...
// chartData only, while the historical rebuild carried it -- so the same profile
// stepped vertically once loaded from history and ramped diagonally while pouring.
...
export function plotHistoricalShot(measurements, workflow = null) {
    ...
    if (workflow && workflow.profile && workflow.profile.steps) {
        ...
        for (const dataPoint of measurements) {
            ...
            // phase.inPour: this branch sat outside the substate check every other
            // trace obeys, so the scale-sourced GFlow line was drawn past where
            // pressure and flow stop -- and on a record whose machine frames carry
            // no substate at all, drawn alone across the whole record (shotEndTime
            // is null there, so the bound below never applied either). The live path
            // and the machine-sourced GFlow are both pouring-only; this was the one
            // source of three that disagreed.
