...
MSL is a **fork of `allofmeng/streamline_project`**, published at `allofmeng/MSL`. Confirmed with the Decaid maintainer: **Decaid will not add MSL to `skin_sources.json`** — no release-based auto-install/upgrade for this skin, ever. That's not a temporary gap to close, it's the permanent distribution model:

- **Distribution**: users get MSL manually from its GitHub releases page — zip download or `POST /webui/skins/install/url` pointed at a release asset. There is no discovery path through Decaid.
- **Update notification**: since Decaid won't track MSL, the Settings-page badge is the only place a user learns a newer version exists. `skinRepoSlug()` in `settings.js` falls back to `allofmeng/MSL` when a skin's install metadata is missing (live-edit, zip installs); release-installed copies get the correct slug from `reaMetadata.sourceUrl` automatically. Either way it feeds `compareVersions(installed, latestTag)` against `allofmeng/MSL`'s own release tags — safe because it's comparing MSL against MSL, not against upstream's differently-numbered version history. Do **not** point it at the upstream repo — a fork restarts its version numbering, so the badge would read "Update available" permanently.

A release zip must still have **`skin-manifest.json` at its root** with `id: "msl"` — that's Decaid's generic skin-install validation (whitelisted paths, no `:` in filenames), independent of `skin_sources.json`. The filename is `skin-manifest.json` rather than `manifest.json` on purpose — `index.html` may ship a PWA `manifest.json` with an incompatible schema; Decaid prefers the skin-specific one and falls back to `manifest.json`, then to the directory name, for the skin id.
